/* Wazp3D Beta 55 : Alain THEIER - Paris - FRANCE - (November 2006 to 2012) 	*/
...
unsigned long PrimitiveGL[7]={GL_TRIANGLES,GL_TRIANGLE_FAN,GL_TRIANGLE_STRIP,GL_POINTS,GL_LINES,GL_LINE_LOOP,GL_LINE_STRIP};
/*=============================================================*/
#ifndef GL_BLEND
...
/*==================================================================*/
void OS_Mouse(void *hc)
{
/* WIN32: Draw a triangle as original amiga mouse */
struct HARD3D_context *HC=hc;
...
/* mesa will use overwin */
	...
	tags[i].ti_Tag = AMA_Window;		tags[i++].ti_Data = (IPTR)mesawin;
	tags[i].ti_Tag = AMA_Left;		tags[i++].ti_Data = x;
	tags[i].ti_Tag = AMA_Top;		tags[i++].ti_Data = y;
	tags[i].ti_Tag = AMA_Width;		tags[i++].ti_Data = large;
	tags[i].ti_Tag = AMA_Height;		tags[i++].ti_Data = high;
	tags[i].ti_Tag = AMA_DoubleBuf;	tags[i++].ti_Data = GL_TRUE;
	tags[i].ti_Tag = AMA_RGBMode;		tags[i++].ti_Data = GL_TRUE;
	tags[i].ti_Tag = AMA_NoDepth;		tags[i++].ti_Data = GL_FALSE;
	tags[i].ti_Tag = AMA_NoStencil;	tags[i++].ti_Data = GL_TRUE;
	tags[i].ti_Tag = AMA_NoAccum;		tags[i++].ti_Data = GL_TRUE;
	tags[i].ti_Tag = TAG_DONE;

/* So Context will be created with our hackrastport->BitMap */
	HC->hglrc = AROSMesaCreateContext(tags);
	if(!HC->hglrc)
...
void OS_CurrentContext(void *hc)
{
/* MORPHOS: set current OpenGL context */
struct HARD3D_context *HC=hc;

	HFUNC(OS_CurrentContext)
	if(hc!=currenthc)
		{
		__tglContext = HC->hglrc;		/* All tinygl.library functions are macros defined this way: #define glFoo(a, b) GLFoo(__tglcontext, a, b) */
		currenthc=hc;
		}
}
/*==================================================================*/
void OS_AdjustWindow(void *hc)
{
/* MORPHOS: Move the "overlay" window like the original amiga window */
struct HARD3D_context *HC=hc;
...
int OS_StartGL(void *hc,void *bm,int x, int y,int large, int high)
{
/* MORPHOS: Open TinyGL & use original warp3D's bm   */
struct HARD3D_context *HC=hc;
struct Window   *mesawin;
struct RastPort *hackrastport;
ULONG width,height,depth,ok;
ULONG Flags =WFLG_ACTIVATE | WFLG_REPORTMOUSE | WFLG_RMBTRAP | WFLG_SIMPLE_REFRESH | WFLG_GIMMEZEROZERO ;
ULONG IDCMPs=IDCMP_CLOSEWINDOW | IDCMP_VANILLAKEY | IDCMP_RAWKEY | IDCMP_MOUSEMOVE | IDCMP_MOUSEBUTTONS ;
...
VAR(TinyGLBase)
	if(!TinyGLBase)
		{ REMP("MORPHOS: Cant open TinyGL.library\n"); return 0; }

/* Current Amiga window : certainly the one we want */
	mesawin=HC->awin;

	REMP("MORPHOS: HC->awin %ld \n",HC->awin);
	REMP("MORPHOS: mesawin <%s> at %ld %ld size %ldX%ld\n",mesawin->Title,mesawin->LeftEdge,mesawin->TopEdge,mesawin->Width,mesawin->Height);
	REMP("MORPHOS: mesawin (borders %ld %ld %ld %ld )\n"  ,mesawin->BorderLeft,mesawin->BorderTop,mesawin->BorderRight,mesawin->BorderBottom);
...
VAR(HC->hglrc)
	if(!HC->hglrc)
		{ REMP("MORPHOS: Cant create TinyGL context\n"); return 0; }
	OS_CurrentContext(hc);

VAR(bm)
	HC->hardbm=NULL;
REM(glAInitializeContextBitMap)
	ok=glAInitializeContextBitMap(bm);		/* use Warp3D's bitmap for MiniGL*/
 VAR(ok)
	if(!ok)
		{ REMP("MORPHOS: Cant glAInitializeContextBitMap\n"); return 0; }


	REMP("MORPHOS: HC->awin %ld HC->hglrc %ld \n",HC->awin,HC->hglrc);
	return 1;
}
/*==================================================================*/
void OS_CloseGL(void *hc)
{
/* MORPHOS: Close TinyGL */
struct HARD3D_context *HC=hc;
...
struct HARD3D_context *HC=hc;
	...
	glFlush();
	glFinish();
	glASwapBuffers(); 						/* else we we drawn in Warp3D's bitmap */

}
/*==================================================================*/
#endif	/* end for morphos */
/*==================================================================*/
...
/* use a flat mode */
	...
	glEnableClientState(GL_VERTEX_ARRAY);		/* allways draw vertex */
...
VAR(gltex)
	...
	level=0;
	glTexImage2D(GL_TEXTURE_2D,level,bits/8,large,high, 0, glformat, GL_UNSIGNED_BYTE,pt);
if(UseMip)
	{
REM(UseMip)
	level=1;
...
VAR(primitive)
	...
	if(state->PerspMode!=0)			/* use persp */
	{
	NLOOP(Pnb)
		{
	...
	primitive=PrimitiveGL[primitive];
...
/* use a flat mode */
	...
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	glViewport(0,0,HC->large,HC->high);
}
/*==================================================================*/
...
/* --- TexEnvMode -----------------*/
	...
	glTexEnvi(GL_TEXTURE_ENV,GL_TEXTURE_ENV_MODE,TexEnvMode32);
	VAR(TexEnvMode32)
	}
...
struct HARD3D_context *HC=hc;
struct HARD3D_texture *HT=ht;
unsigned long glformat,x,y,level,size;
unsigned char UseMip;
GLuint gltex;

	HFUNC(HARD3D_UpdateTexture);
	OS_CurrentContext(hc);
	gltex=HT->gltex;
	UseMip=HT->TexFlags AND 1;
	if(large!=high)
		UseMip=FALSE;
	...
	x=y=level=0;
	if(bits==32)
		glformat=GL_RGBA; else glformat=GL_RGB;

	glTexSubImage2D(GL_TEXTURE_2D,level,x,y,large,high,glformat,GL_UNSIGNED_BYTE,pt);
if(UseMip)
	{
REM(UseMip)
	level=1;
